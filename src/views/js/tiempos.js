...
function updateStatus(rows) {
  ...
  if (left) left.innerHTML = `<span class="status-filter-icon">F</span>Filtrando: ${tareaTxt} · Asignada a ${asignadaNombre || '—'}`;

  const right = status.querySelector('span:last-child');
